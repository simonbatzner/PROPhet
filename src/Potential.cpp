...
Potential::Potential(vector < System * > systems_in, Functional_params
F_in) :

systems (systems_in), params(F_in) {
...
//      cout << "############" << endl;
    ...
    for (int i = 0; i < atom_types.size(); i++) {
        ...
        if (!F_in.input_file().empty()) {
            ...
            if (!file.is_open()) {
                ERROR(
                        "Found atom type " + atom.atomic_symbol() + " but could not open restart file '" + filename +
                        "'");
            }
            if (mpi->io_node()) {
...
void Potential::forces() {
    ...
    for (int i_sys = 0; i_sys < systems.size(); i_sys++) {
        ...
        for (int atom = 0; atom < systems[i_sys]->structure.Natom; atom++) {
            ...
            my_outputs[atom] += this->evaluate_MD(atom, systems[i_sys]->structure.types[atom].atomic_number(),
                                                  dE_dG[atom]);
        }
        ...
        for (int atom = 0; atom < systems[i_sys]->structure.pos.size(); atom++) {
            fout << systems[i_sys]->structure.types[atom].atomic_symbol() << "  " << setw(w) << my_outputs[atom] << " "
                 << setw(w) << f[atom][0] << " " << setw(w) << f[atom][1] << " " << setw(w) << f[atom][2] << endl;
        }
        fout.close();
    }
...
void Potential::validate() {
    ...
    if (mpi->io_node()) {
        ...
        for (int i = 0; i < system_map.size(); i++) {
            int index = system_map[i];
            if (unrav) {
                cout << count++ << "               " << unraveled.at(index) << setw(25) << unTargets.at(index)
                     << setw(15) << nat.at(index) << endl;
                ...
                cout << count++ << "              " << output.at(index) + output_mean << "              "
                     << targets.at(index) << "      " << nat.at(index) << endl;
